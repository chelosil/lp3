...
if ($exite) {
  $query = mysqli_query($conexion, "INSERT INTO persona (id, nombre, apellido) VALUES (null, '$nombre', '$apellido')") or die('error: ' . mysqli_error($conexion));
  if ($query) {
...
elseif(isset($_POST['EnviarEditar'])){
    ...
    apellido='$apellido' WHERE id=$id") or die('error: '. mysqli_error($conexion));
    if($query){
        echo "La modificación de datos fue exitosa"; ?>
        <meta http-equiv="refresh" content="5, url=http://localhost/lp3/php/pruebamysqlphp/index.php">
        <?php } else {
